...
static void settings_init(void) {
    ...
    settings.rdma = false;
    settings.rport = 12345;
    /* By default this string should be NULL for getaddrinfo() */
    settings.inter = NULL;
    settings.maxbytes = 1024 * 1024 * 1024; /* default is 1 GB */
...
static void conn_init(void) {
    ...
    if(settings.rdma) {
    	char port[sizeof(int)*3+2];
    	snprintf(port, sizeof port, "%d", settings.rport);
    	//run_offload_server(assoc2_hash_start(), assoc2_hash_size(), port);
	run_offload_server(slabs_mem_base(), slabs_mem_limit(), port);
//uintptr_t slabs_mem_base();
//size_t slabs_mem_limit();
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("rdma", "%d", settings.rdma);
    APPEND_STAT("rport", "%d", settings.rport);
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
	    ...
	    case ENABLE_RDMA:
	         if (subopts_value == NULL) {
                    fprintf(stderr, "Missing numeric argument for rdma port\n");
                    return 1;
                }
		settings.rdma = true;
		settings.rport = atoi(subopts_value);
		break;
            default:
    ...
    /* if (start_assoc2_maintenance_thread() == -1) { */
    /*     exit(EXIT_FAILURE); */
    /* } */
    ...
    //stop_assoc2_maintenance_thread();
