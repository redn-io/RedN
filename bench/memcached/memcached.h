...
typedef struct _slabbed_item {
    ...
    uint8_t         slabs_clsid;/* which slab class we're in */
    uint8_t         nkey;       /* key length, w/terminating null and padding */
    // Bin: two pointers to maintain free list
...
enum store_item_type do_store_item(item *item, const uint32_t hv);
...
//enum store_item_type item_store(item *item);
...
static inline TagType _tag_hash(const uint32_t hv) {
    uint32_t r =  hv & tagmask;
    return (TagType) r + (r == 0);
}

static inline size_t _alt_index(const size_t index, const TagType tag) {
    // 0x5bd1e995 is the hash constant from MurmurHash2
    return (index ^ ((tag & tagmask ) * 0x5bd1e995)) & hashmask;
}

static inline size_t _lock_index(const size_t i1, 
                                 const size_t i2,
                                 const TagType tag) {
    /* if ((tag & locmask) == 0) { */
    /*     return i1; */
