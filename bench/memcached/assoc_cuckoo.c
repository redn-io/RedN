...
void assoc2_init(const int hashtable_init) {
...
#if 0
    int ret = posix_memalign(&ptr, sysconf(_SC_PAGESIZE), hashsize * sizeof(struct Bucket));
    ...
    ptr = memory_allocate(hashsize * sizeof(struct Bucket));

    buckets = (struct Bucket *) ptr;
...
#ifdef REDN
 		char* it_key = (char*) ITEM_key(it);
		strncpy(buckets[i].key, it_key, 1);
		
		printf("key %x address %lu start_address %lu i %lu\n", (uint8_t)*buckets[i].key,
				(uintptr_t)buckets[i].key, assoc2_hash_start(), i);
		buckets[i].addr = htobe64((uintptr_t) ITEM_data(it));

		char *test = (char*) be64toh(buckets[i].addr);
...
uintptr_t assoc2_hash_start()
{
	return (uintptr_t) buckets;
}

uint64_t assoc2_hash_size()
{
	return hashsize * sizeof(struct Bucket);
}
